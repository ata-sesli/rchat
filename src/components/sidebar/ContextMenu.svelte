...
{#if show}
  <div
    class="fixed z-[100] bg-theme-base-800 border border-theme-base-700 rounded-lg shadow-xl py-1 min-w-[140px] animate-fade-in-up"
  ...
  >
    {#if target?.type === "peer"}
      ...
      {#if currentEnvelope}
        <button
          ...
          class="w-full text-left px-4 py-2 text-sm text-theme-error-400 hover:bg-red-500/10 transition-colors"
        >
    ...
    {:else if target?.type === "envelope"}
      ...
      <button
        ...
        class="w-full text-left px-4 py-2 text-sm text-theme-base-400 hover:bg-theme-base-700 hover:text-white transition-colors"
      >
        More...
      </button>
    {/if}
