...
After bootstrap, launch the GUI with `bun run tauri dev`. For distributable GUI/TUI packages, use the [runtime packaging scripts and validation checklist](scripts/dist/PACKAGING.md), which enforce build baselines and inspect linked libraries. The TUI is a separate Rust binary and launches in Kitty, which is not installed by the source bootstrap. Install Kitty or set `RCHAT_KITTY_PATH` to its executable. The launcher also checks the saved Media settings path, a sibling executable, `PATH`, and the standard macOS application locations. Use `--no-host` to stay in the current terminal; SSH sessions also stay in their current terminal. Fast image viewing and persistent video-frame updates require Kitty. Kitty does not have a native Windows application.
